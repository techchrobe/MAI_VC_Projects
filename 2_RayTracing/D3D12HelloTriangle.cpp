...
// Following Code is from the DX12 Raytracing Tutorial
...
AccelerationStructureBuffers D3D12HelloTriangle::CreateBottomLevelAS(std::vector<std::pair<ComPtr<ID3D12Resource>, uint32_t>> vVertexBuffers)
{
	...
	// Build the acceleration structure. Note that this call integrates a barrier on the generated AS, so that it can be used to compute a top-level AS right after this method. 
...
void D3D12HelloTriangle::CreateTopLevelAS(const std::vector<std::pair<ComPtr<ID3D12Resource>, DirectX::XMMATRIX>>& instances)
{
	...
	// The buffer describing the instances: ID, shader binding information, matrices ... Those will be copied into the buffer by the helper through mapping, so the buffer has to be allocated on the upload heap.
	...
	// After all the buffers are allocated, or if only an update is required, we can build the acceleration structure. Note that in the case of the update we also pass the existing AS as the 'previous' AS, so that it can be refitted in place.
...
void D3D12HelloTriangle::CreateAccelerationStructures()
{
	...
	m_commandQueue->ExecuteCommandLists(1, ppCommandLists); m_fenceValue++;
...
void D3D12HelloTriangle::CreateRaytracingPipeline()
{
	...
	// This as to be done explicitly in the lines below.
	...
	// Upon hitting a surface, DXR can provide several attributes to the hit. In our sample we just use the barycentric coordinates defined by the weights u,v of the last two vertices of the triangle.
...
void D3D12HelloTriangle::UpdateCameraBuffer()
{
	...
	// The lookat and perspective matrices used for rasterization are defined to transform world-space vertices into a [0,1]x[0,1]x[0,1] camera space
	...
	// To do this, we need to store the inverse matrices as well.
	XMVECTOR det;
